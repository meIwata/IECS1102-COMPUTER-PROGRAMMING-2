    ...
    */
    public Pokemon fight(Pokemon pokemon1, Pokemon pokemon2) {
        ...
        if (pokemon1.getLevel() > pokemon2.getLevel()) {
            ...
            int xPower = pokemon1.attackPower - pokemon1.defencePower;
            int yPower = pokemon2.attackPower - pokemon2.defencePower;
            if (xPower > yPower) {
                System.out.println(pokemon1.getName() + "勝");
                winner = pokemon1;
            } else if (xPower < yPower) {
                ...
                // 隨機挑出 x 或 y
                String[] chosenArray = {pokemon1.getName(), pokemon2.getName()};
                Random random = new Random();
                int index = random.nextInt(2); // 產生一個 0 或 1 的隨機整數
//                System.out.println(chosenArray[index] + "勝");
    ...
    */
    public void fight(Pokemon... pokemons) {
        Pokemon winner = pokemons[0];
        for (int i = 0; i < pokemons.length - 1; i++) {
            winner = fight(winner, pokemons[i + 1]);
        }
        System.out.println("Among the " + pokemons.length + " Pokemons, the winner is ");
        winner.printInfo();
    }
